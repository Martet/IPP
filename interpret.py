...
def main():
    ...
    instrs, labels = parseXML(source)

    i = 0
    while i < len(instrs):
        instr = instrs[i]
        if instr['opcode'] == 'MOVE':
            setVarValue(instr['args'][0], getArgValue(instr['args'][1]))
            ...
            if instr['args'][0]['value'] not in labels:
                error(52)
            i = labels[instr['args'][0]['value']]
            continue
            ...
            stackData.append(getArgValue(instr['args'][0]))
        elif instr['opcode'] == 'POPS':
            setVarValue(instr['args'][0], getStackTop())
        elif instr['opcode'] == 'CLEARS':
            stackData = []
        elif instr['opcode'] in ['ADD', 'SUB', 'MUL', 'DIV', 'IDIV', 'ADDS', 'SUBS', 'MULS', 'DIVS', 'IDIVS']:
            operation(instr)
        elif instr['opcode'] in ['LT', 'GT', 'LTS', 'GTS']:
            compare(instr)
        elif instr['opcode'] in ['EQ', 'EQS']:
            equal(instr)
        elif instr['opcode'] in ['AND', 'OR', 'ANDS', 'ORS']:
            symb2 = getStackTop() if isStackOp(instr) else getArgValue(instr['args'][2])
            symb1 = getStackTop() if isStackOp(instr) else getArgValue(instr['args'][1])
            if symb1['type'] != 'bool' or symb2['type'] != 'bool':
                error(53)
            result = {'type': 'bool', 'value': symb1['value'] and symb2['value'] if instr['opcode'] in ['AND', 'ANDS'] else symb1['value'] or symb2['value']}
            if isStackOp(instr):
                stackData.append(result)
            else:
                setVarValue(instr['args'][0], result)
        elif instr['opcode'] in ['NOT', 'NOTS']:
            symb1 = getStackTop() if isStackOp(instr) else getArgValue(instr['args'][1])
            if symb1['type'] != 'bool':
                error(53)
            if isStackOp(instr):
                ...
                setVarValue(instr['args'][0], {'type': 'bool', 'value': not symb1['value']})
        elif instr['opcode'] in ['FLOAT2INT', 'FLOAT2INTS', 'INT2FLOAT', 'INT2FLOATS']:
            symb1 = getStackTop() if isStackOp(instr) else getArgValue(instr['args'][1])
            type = 'float' if instr['opcode'] in ['FLOAT2INT', 'FLOAT2INTS'] else 'int'
            if symb1['type'] != type:
                error(53)
            if isStackOp(instr):
                stackData.append({'type': 'int' if type == 'float' else 'float', 'value': int(symb1['value']) if type == 'float' else float(symb1['value'])})
            else:
                setVarValue(instr['args'][0], {'type': 'int' if type == 'float' else 'float', 'value': int(symb1['value']) if type == 'float' else float(symb1['value'])})
        elif instr['opcode'] in ['INT2CHAR', 'INT2CHARS']:
            symb1 = getStackTop() if isStackOp(instr) else getArgValue(instr['args'][1])
            if symb1['type'] != 'int':
                error(53)
            try:
                if isStackOp(instr):
                    ...
                    setVarValue(instr['args'][0], {'type': 'string', 'value': chr(symb1['value'])})
            except ValueError:
                error(58)
        elif instr['opcode'] in ['STRI2INT', 'STRI2INTS']:
            symb2 = getStackTop() if isStackOp(instr) else getArgValue(instr['args'][2])
            symb1 = getStackTop() if isStackOp(instr) else getArgValue(instr['args'][1])
            ...
            try:
                ...
                if isStackOp(instr):
                    ...
                    setVarValue(instr['args'][0], {'type': 'int', 'value': ord(symb1['value'][symb2['value']])})
            except IndexError:
                error(58)
        elif instr['opcode'] == 'READ':
            symb1 = getArgValue(instr['args'][1])
            if symb1['type'] != 'type' or symb1['value'] not in ['int', 'float', 'string', 'bool']:
                error(53)
            read = inp.readline().rstrip('\n')
            try:
                if symb1['value'] == 'int':
                    setVarValue(instr['args'][0], {'type': 'int', 'value': int(read)})
                elif symb1['value'] == 'float':
                    setVarValue(instr['args'][0], {'type': 'float', 'value': parseFloat(read)})
                elif symb1['value'] == 'string':
                    setVarValue(instr['args'][0], {'type': 'string', 'value': read})
                elif symb1['value'] == 'bool':
                    setVarValue(instr['args'][0], {'type': 'bool', 'value': read.lower() == 'true'})
            except Exception:
                setVarValue(instr['args'][0], {'type': 'nil', 'value': 'nil'})
        elif instr['opcode'] == 'WRITE':
            symb1 = getArgValue(instr['args'][0])
            if symb1['type'] == 'nil':
                out = ''
            ...
            symb1 = getArgValue(instr['args'][1])
            symb2 = getArgValue(instr['args'][2])
            if symb1['type'] != 'string' or symb2['type'] != 'string':
                error(53)
            setVarValue(instr['args'][0], {'type': 'string', 'value': symb1['value'] + symb2['value']})
        elif instr['opcode'] == 'STRLEN':
            symb1 = getArgValue(instr['args'][1])
            if symb1['type'] != 'string':
                error(53)
            setVarValue(instr['args'][0], {'type': 'int', 'value': len(symb1['value'])})
            ...
            symb0 = getArgValue(instr['args'][0])
            symb1 = getArgValue(instr['args'][1])
            symb2 = getArgValue(instr['args'][2])
            ...
            try:
                ...
                setVarValue(instr['args'][0], {'type': 'string', 'value': out})
            except Exception:
                error(58)
        elif instr['opcode'] == 'TYPE':
            if instr['args'][1]['type'] == 'var':
                var = instr['args'][1]['value'].split('@')
                try:
                    if var[0] == 'GF':
                ...
                setVarValue(instr['args'][0], {'type': 'string', 'value': instr['args'][1]['type']})
            ...
            try:
                ...
                error(52)
            continue
        elif instr['opcode'] in ['JUMPIFEQ', 'JUMPIFNEQ', 'JUMPIFEQS', 'JUMPIFNEQS']:
            symb0 = getArgValue(instr['args'][0])
            symb1 = getStackTop() if isStackOp(instr) else getArgValue(instr['args'][1])
            symb2 = getStackTop() if isStackOp(instr) else getArgValue(instr['args'][2])
            ...
            if instr['opcode'] in ['JUMPIFNEQ', 'JUMPIFNEQS']:
                out = not out
            if out:
                i = labels[symb0['value']]
                continue
